...
namespace inference::gpu::kernels {
    namespace detail {
        struct AddBf16 {
            ...
            __device__ void operator()(const Configuration& config, const TensorView<const __nv_bfloat16> lhs, const TensorView<const __nv_bfloat16> rhs,
                ...
                for (std::size_t index = thread_index; index < element_count; index += thread_count) {
                    output[index] = __float2bfloat16(__bfloat162float(lhs[index]) + __bfloat162float(rhs[index]));
                }
            }
        };
    ...
    inline void add(const cuda::stream_ref stream, TensorView<const __nv_bfloat16> lhs, TensorView<const __nv_bfloat16> rhs,
                    TensorView<__nv_bfloat16> output) {
        const auto element_count = output.size();
        ...
        cuda::launch(stream, config, detail::AddBf16{}, lhs, rhs, output);
    }
