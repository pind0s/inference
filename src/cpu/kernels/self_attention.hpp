...
namespace inference::cpu::kernels {
    inline void self_attention(const bf16_t* __restrict query, const bf16_t* __restrict key, const bf16_t* __restrict value,
                               bf16_t* __restrict output, const std::size_t sequence_size, const std::size_t query_head_count,
        ...
        std::vector<float> scores(sequence_size);
        for (std::size_t query_position = 0; query_position < sequence_size; ++query_position) {
            for (std::size_t query_head = 0; query_head < query_head_count; ++query_head) {
                ...
                const auto query_base = (query_position * query_head_count + query_head) * head_size;
