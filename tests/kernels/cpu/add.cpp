...
TEST_P(AddTest, MatchesReference) {
    ...
    std::vector<cpu::bf16_t> reference_output(element_count);
    std::vector<cpu::bf16_t> actual_output(element_count);

    test::reference::cpu::add(lhs.data(), rhs.data(), reference_output.data(), element_count);
    ...
    for (const auto [reference, actual] : std::views::zip(reference_output, actual_output)) {
        EXPECT_FLOAT_EQ(reference.to_float(), actual.to_float());
    }
}
